									...
									.then(user => {
											...
											as= user.role.indexOf('-')>0? Helpers.capitalize(user.role.split('-')[0]+' '+Helpers.capitalize(user.role.split('-')[1])): Helpers.capitalize(user.role);
                                                ...
                                                let mailOptions = {
													...
													as + '<p>Login with the following details: ' +'<p><b>Email</b>: ' 
													+ user.email+'</p><p> <b>Password</b>: '+ sendpasssword+'</p>'
                                                    };
                                                    smtpTransport.sendMail(mailOptions,(err,info)=>{
                                                        if(err){
                                                            return res.status(400).json({success:false,message:err.message});
                                                        }else{
                                                           return res.status(200).json({success:true,message:'Registration successful.An email has been sent to the new user for login details!'});
                                                        }
                                                    });
                                            });
									} 
...
**/
router.post('/all',  passport.authenticate('jwt', { session: false }),Middleware.isManager ,(req,res,next)=>{
	const { body } = req;
	const { user } = req;
	let st = [
...
router.patch('/password', passport.authenticate('jwt', { session: false }), Middleware.isManager, (req, res, next) => {
  const { body } = req;
  const _id = body._id;
